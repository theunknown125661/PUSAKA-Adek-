...
BEGIN
  ...
  -- 2. Act when status changes TO absent (explicitly marked absent)
  ELSIF NEW.status = 'absent' AND (OLD.status IS NULL OR OLD.status != 'absent') THEN
    
    IF v_streak_record.shield_count > 0 THEN
       -- Has a shield to absorb the absence
       UPDATE public.streaks SET
         shield_count = v_streak_record.shield_count - 1,
         shield_used_dates = array_append(v_streak_record.shield_used_dates, v_current_date),
         -- We update last_attendance_date to act as a bridge, so tomorrow it sees 0 eligible days missed
         last_attendance_date = v_current_date, 
         updated_at = now()
       WHERE id = v_streak_record.id;
    ELSE
       -- No shield, completely break the streak!
       UPDATE public.streaks SET
         current_streak = 0,
         updated_at = now()
       WHERE id = v_streak_record.id;
