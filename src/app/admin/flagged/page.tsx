    ...
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-xl font-bold flex items-center gap-2 text-orange-500">
          <AlertTriangle className="h-5 w-5" /> Flagged Cases
        </h1>
        <p className="text-muted-foreground text-sm mt-1">Review {logs.length} cases escalated by teachers</p>
      </div>

      {logs.length === 0 ? (
        <EmptyState icon={AlertTriangle} title="No flagged cases" description="There are no escalated cases requiring your attention." />
      ) : (
            ...
            <div key={log.id} className="glass border-orange-500/20 rounded-xl p-5 space-y-4">
              ...
              <div className="bg-orange-500/10 rounded-xl p-4 border border-orange-500/20 space-y-2">
                <p className="text-xs font-semibold text-orange-500 flex items-center gap-1">
                  <AlertTriangle className="h-3.5 w-3.5" /> ESCALATION NOTE
                </p>
                ...
                <div className="flex-1 space-y-2">
                  ...
                  {log.fraud_flags && log.fraud_flags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {log.fraud_flags.map((f) => <span key={f} className="px-2 py-1 rounded-md bg-destructive/10 text-destructive text-[10px] font-medium">{flagLabel(f as FraudFlag)}</span>)}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-2 items-end pt-2 border-t border-border/50">
                <input 
                  ...
                  placeholder="Final admin decision note" 
                ...
                <button onClick={() => handleAction(log.id, "approved")} disabled={processingId === log.id} className="px-4 py-2 rounded-lg bg-emerald-500 text-white text-xs font-medium flex items-center gap-1.5 hover:bg-emerald-600 disabled:opacity-50">
                  {processingId === log.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <CheckCircle className="h-3.5 w-3.5" />} Overrule & Approve
                </button>
